...
def open_sequence_denoiser(seq_dir, attacked_obs, gray_mode, expand_if_needed=False, max_num_fr=100):
	r""" Opens a sequence of images and expands it to even sizes if necesary
	Args:
		seq_dir: 1,3,448,448
		attacked_obs: 448,448,3
		gray_mode: boolean, True indicating if images is to be open are in grayscale mode
		expand_if_needed: if True, the spatial dimensions will be expanded if
			size is odd
		expand_axis0: if True, output will have a fourth dimension
	...
	i = 0
	while i<max_num_fr:
            img, expanded_h, expanded_w = open_image_denoiser(files, attacked_obs, gray_mode=gray_mode, expand_if_needed=expand_if_needed, expand_axis0=False)
...
def open_image_denoiser(fpath, attacked_obs, gray_mode, expand_if_needed=False, expand_axis0=True, normalize_data=True):
	r""" Opens an image and expands it if necesary
	Args:
		fpath: 1,3,448,448
		attacked_obs: attacked image shape is 448,448,3
		gray_mode: boolean, True indicating if image is to be open
			in grayscale mode
		expand_if_needed: if True, the spatial dimensions will be expanded if
			size is odd
	...
	"""
	if not gray_mode:
		# Open image as a HxWxC torch.Tensor
		img = attacked_obs
		# from HxWxC to CxHxW, RGB image
		img = (cv2.cvtColor(img, cv2.COLOR_BGR2RGB)).transpose(2, 0, 1)
	else:
