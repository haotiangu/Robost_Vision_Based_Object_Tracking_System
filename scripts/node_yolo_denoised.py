...
if __name__=='__main__':
    ...
    sub_denframe_image = rospy.Subscriber('/fastdvdnet_node/clean_image', Image, fnc_callback7)
    ...
    while not rospy.is_shutdown():
        ...
        if IMAGE_RECEIVED is not None:
            if ATTACKED_IMAGE is not None and DENFRAME_IMAGE is None and IMAGE_ATTACK_ON_CMD_RECEIVED is not None and IMAGE_ATTACK_ON_CMD_RECEIVED.data:
                np_im = np.frombuffer(ATTACKED_IMAGE.data, dtype=np.uint8).reshape(ATTACKED_IMAGE.height, ATTACKED_IMAGE.width, -1)
            elif ATTACKED_IMAGE is not None and DENFRAME_IMAGE is not None and IMAGE_ATTACK_ON_CMD_RECEIVED is not None and IMAGE_ATTACK_ON_CMD_RECEIVED.data:
                np_im = np.frombuffer(DENFRAME_IMAGE.data, dtype=np.uint8).reshape(DENFRAME_IMAGE.height, DENFRAME_IMAGE.width, -1)
            else:
